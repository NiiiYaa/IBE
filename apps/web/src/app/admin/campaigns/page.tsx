...
import type { Campaign, CreateCampaignRequest, UpdateCampaignRequest } from '@ibe/shared'
import { apiClient } from '@/lib/api-client'
import { useAdminProperty } from '../property-context'

function effectiveActive(a: Campaign): boolean {
...
function campaignUrl(code: string): string {
  return `${window.location.origin}/search?campaignId=${code}`
}
...
export default function CampaignsPage() {
  ...
  return <CampaignsEditor propertyId={propertyId} />
}

function CampaignsEditor({ propertyId }: { propertyId: number | null | undefined }) {
  ...
  const queryKey = useMemo(() => ['campaigns', scopedPropertyId ?? 'global'], [scopedPropertyId])

  const { data: campaigns = [], isLoading } = useQuery<Campaign[]>({
    queryKey,
    queryFn: () => apiClient.listCampaigns(scopedPropertyId),
  ...
  const { mutate: save, isPending: isSaving } = useMutation({
    mutationFn: (payload: CreateCampaignRequest | { id: number; data: UpdateCampaignRequest }) => {
      if ('id' in payload) return apiClient.updateCampaign(payload.id, payload.data)
  ...
  const { mutate: toggleOverride } = useMutation({
    mutationFn: ({ id, isEnabled }: { id: number; isEnabled: boolean }) =>
      apiClient.setPropertyOverride({ entityType: 'campaign', entityId: id, propertyId: scopedPropertyId!, isEnabled }),
    onMutate: ({ id, isEnabled }) => {
      const prev = qc.getQueryData<Campaign[]>(queryKey)
      qc.setQueryData<Campaign[]>(queryKey, old => old?.map(a => a.id === id ? { ...a, propertyEnabled: isEnabled } : a) ?? [])
      return { prev }
    },
    onError: (_, __, ctx) => {
  ...
  function handleEdit(a: Campaign) {
    setEditingId(a.id)
    setSaveError(null)
    setForm({
      code: a.code,
  ...
  function handleCopyUrl(a: Campaign) {
    copyToClipboard(campaignUrl(a.code))
    setCopiedId(a.id)
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-[var(--color-text)]">Campaigns</h1>
        {scopedPropertyId == null
          ...
          : <p className="mt-1 text-sm text-[var(--color-text-muted)]">Property-specific campaigns plus inherited global campaigns.</p>}
      </div>
      ...
      <div className="mb-8 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          {/* Code */}
          <div>
            <label className={labelCls}>Campaign code <span className="font-normal normal-case text-[var(--color-text-muted)]/60">used in URLs</span></label>
          ...
          {/* Media */}
          <div>
            <label className={labelCls}>Media <span className="font-normal normal-case text-[var(--color-text-muted)]/60">optional</span></label>
            <input
              type="text"
              value={form.email}
              onChange={e => set('email', e.target.value)}
              placeholder="e.g. Google Ads, Facebook, Newsletter"
              className={inputCls}
            />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => set('hasCommission', false)} className={toggleCls(!form.hasCommission)}>None</button>
              <button type="button" onClick={() => set('hasCommission', true)} className={toggleCls(form.hasCommission)}>Set rate</button>
            </div>
            {form.hasCommission && (
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => set('hasDiscount', false)} className={toggleCls(!form.hasDiscount)}>None</button>
              <button type="button" onClick={() => set('hasDiscount', true)} className={toggleCls(form.hasDiscount)}>Set discount</button>
            </div>
            {form.hasDiscount && (
              <div className="mt-2 flex items-center gap-3">
                <input
        ...
        <div className="mt-5 flex gap-2">
          ...
          >
            {isSaving ? 'Saving…' : isEditing ? 'Save changes' : 'Add campaign'}
          </button>
          {isEditing && (
            <button
              type="button"
