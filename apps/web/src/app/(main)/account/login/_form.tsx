...
function LoginFormInner({ propertyId }: { propertyId: number }) {
  ...
  async function handleForgot(e: React.FormEvent) {
    ...
    await apiClient.guestForgotPassword(forgotEmail.trim(), propertyId).catch(() => {})
          ...
          forgotDone ? (
            ...
            <form onSubmit={handleForgot} className="space-y-4">
              ...
              <div>
                ...
                <input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} required autoComplete="email" className="w-full rounded-md border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2 text-sm text-[var(--color-text)] outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary-light)]" />
              </div>
