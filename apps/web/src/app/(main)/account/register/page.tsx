...
function RegisterForm() {
  ...
  const [form, setForm] = useState({ email: '', password: '', firstName: '', lastName: '', phone: '', nationality: '' })
    ...
    <main className="flex min-h-[70vh] items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-8 shadow-sm">
        ...
        {googleEnabled && (
          <>
            <a
              href={`/api/v1/guest/auth/google?state=${PROPERTY_ID}`}
        ...
        <p className="mt-5 text-sm text-[var(--color-text-muted)]">
          ...
          <Link href="/account/login" className="font-medium text-[var(--color-primary)] hover:underline">
            Sign in
          </Link>
        </p>
