...
async function fetchIncentive(propertyId: number): Promise<IncentiveDisplay | null> {
  try {
    const res = await fetch(`${API_URL}/api/v1/incentives/property/${propertyId}`)
    if (!res.ok) return null
    return res.json() as Promise<IncentiveDisplay | null>
  } catch { return null }
}

export function useIncentive(propertyId: number | null) {
  return useQuery<IncentiveDisplay | null>({
