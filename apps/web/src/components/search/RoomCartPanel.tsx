  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-2xl">
      ...
      <div className="bg-primary px-5 py-3 shrink-0">
        <p className="text-xs font-medium uppercase tracking-wider text-white/70">Your selection</p>
        <p className="text-base font-semibold text-white">
          {formatCurrency(total, displayCurrency, locale)} total
        </p>
        {nights > 0 && (
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-[var(--color-border)]">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-5 py-10 text-center">
            ...
            <p className="text-sm font-medium text-[var(--color-text)]">No rooms added yet</p>
            <p className="mt-1 text-xs text-[var(--color-text-muted)]">Select a room from the list to add it here.</p>
          </div>
        ) : (
          items.map((item, idx) => (
