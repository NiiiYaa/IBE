# Admin Invitation Search Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Upgrade the admin Hotel Onboarding page from a static URL form to a two-step search-and-pick flow (hotel name + city → results grid with screenshots → select URL → create invitation).

**Architecture:** A new proxy endpoint in `apps/api` forwards hotel search requests to `onboarding-api`'s existing `/hotel-search` route. The admin page (`apps/web`) gains a two-step form: Step 1 searches and shows a screenshot grid, Step 2 collects email + CM and creates the invitation with the discovered `ibeUrl`.

**Tech Stack:** TypeScript, React (Next.js 14), Fastify

---

## Task 1: API proxy — `POST /admin/hotel-onboarding/search`
...
In `apps/api/src/routes/onboarding-admin.route.ts`, add this route **after the existing `GET /admin/hotel-onboarding/invitations` route** (after line 44):
...
git commit -m "feat(api): add hotel search proxy for admin invitation flow"
```

---

## Task 2: apiClient method + env var

**Files:**
- Modify: `apps/web/src/lib/api-client.ts`
- Modify: `apps/web/.env.local`

- [ ] **Step 1: Add `searchOnboardingHotel` to apiClient and fix `ibeUrl`**

In `apps/web/src/lib/api-client.ts`, in the Self-Onboarding section (around line 2574):

**Add** `searchOnboardingHotel` after `listOnboardingInvitations`:
```typescript
...
**Change** `createOnboardingInvitation` — replace `websiteUrl` with `ibeUrl` in the signature:
```typescript
...
In `apps/web/.env.local`, add at the end:
```
NEXT_PUBLIC_ONBOARDING_API_URL=http://localhost:3003
```
...
```bash
git add apps/web/src/lib/api-client.ts apps/web/.env.local
git commit -m "feat(web): add searchOnboardingHotel apiClient method, fix ibeUrl param"
```

---

## Task 3: Admin page — two-step invitation form
...
Replace the full contents of `apps/web/src/app/admin/hotel-onboarding/page.tsx` with:
...
git commit -m "feat(web): admin invitation — two-step search+pick+create flow"
```
